...
export default function land(args) {
  ...
  const receipt = renderReceipt(dock, route, head);
...
const ROUTE_MARKER = '**drydock-route:v1**';

function renderReceipt(dock, route, head) {
  const verdicts = route.gates.map((n) => ({ name: n, g: dock.gates[n], agent: isAgent(dock.gates[n].by) }));
